...
namespace MultiBoost {
                              ...
                              vector<RawData::eAttributeType>& attributeTypes )
    {
        ...
        for (int i = 0; i < _sepChars.size(); i++) {
            std::cout << "SepChar: i-" << i << " value:"<< (int)_sepChars[i] << std::endl;
        }
        ...
        for (i = 0; i < numRows; ++i)
        {
            ...
            if (_hasClassEnd)
                inFile >> tmpClassName; // store class
//            std::cout << "Read Row:" << i << " ExpName:" << tmpExampleName << " ClassName:" << tmpClassName << std::endl;
...
//            currExample.addBinaryLabel(classIdx);
    ...
    {
        if (numColumns <= 0) {
            cerr << "Data Input columns:" << numColumns << endl;
            return false;
        }
